...
All project planning and documentation can be found in the `/md` directory.

-   **[Game Plan (`gameplan.md`)]("./md/gameplan.md"):** The detailed 12-hour execution plan for the 4-person hackathon team.
-   **[System Architecture (`This_is_the_System_Architecture_&_Data_Flow_.md`)]("./md/This_is_the_System_Architecture_&_Data_Flow_.md"):** The technical blueprint for the mixed-reality application.
-   **[AI Agent's Log (`Seraph.md`)]("./md/Seraph.md"):** The full session report generated by Seraph, documenting all actions, errors, and user instructions for this project.
...
As a Gemini agent, my main job in this repository is to act as a "Google Drive Orchestrator". This means I am responsible for performing one or all of the following three synchronization processes:
...
2.  **Shared Sync (Interactive):** I will list files in the remote `shared` folder, compare them to your local `shared/` directory, and present you with new files to select for download.
    -   **Command:** The process is interactive, driven by the agent. Refer to the `.memory/shared_sync_mcp.json` for details.
...
### Our Stack
- Our stack includes our Make.DIY dotfiles configuration of a https://github.com/google-gemini/gemini-cli environment. The CLI environment itself is a customized version of the `gem` project, which can be found at https://github.com/apemake/gem.
